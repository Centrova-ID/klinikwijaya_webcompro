        ...
        <div class="max-w-3xl mx-auto px-8">
            <form id="event-form" action="{{ route('admin.events.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-2 gap-4 mb-8">
                    ...
                    <div>
                        <input type="text" name="location" id="location" value="{{ old('location') }}" placeholder="Lokasi event..."
                            class="text-sm text-gray-500 border-0 border-b border-gray-200 focus:border-[#3F5499] focus:ring-0 px-0 py-2 bg-transparent w-full">
                        @error('location')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                ...
                <div class="mb-8">
                    <textarea 
                        ...
                        maxlength="440"
                        rows="2"
                        class="w-full text-xl text-gray-600 border-0 focus:ring-0 p-0 placeholder-gray-300 resize-none leading-snug overflow-hidden outline-none">{{ old('description') }}</textarea>
                    <div class="flex justify-between items-center mt-1">
                        @error('description')
                            <p class="text-red-500 text-sm">{{ $message }}</p>
                        @enderror
