            ...
            <form id="article-form" action="{{ route('admin.articles.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    <textarea 
                        ...
                        required
                        rows="1"
                        class="w-full text-5xl font-bold text-gray-900 border-0 focus:ring-0 p-0 placeholder-gray-300 leading-tight tracking-tight resize-none overflow-hidden outline-none">{{ old('title') }}</textarea>
                    <div class="flex justify-between items-center mt-1">
                        @error('title')
                            <p class="text-red-500 text-sm">{{ $message }}</p>
                        @enderror
                        <span id="title-counter" class="text-xs text-gray-400 ml-auto">0/50</span>
                    </div>
                </div>
                ...
                <div class="mb-8">
                    <textarea 
                        ...
                        maxlength="440"
                        rows="2"
                        class="w-full text-xl text-gray-600 border-0 focus:ring-0 p-0 placeholder-gray-300 resize-none leading-snug overflow-hidden outline-none">{{ old('excerpt') }}</textarea>
                    <div class="flex justify-between items-center mt-1">
                        @error('excerpt')
                            <p class="text-red-500 text-sm">{{ $message }}</p>
                        @enderror
                        <span id="excerpt-counter" class="text-xs text-gray-400 ml-auto">0/125</span>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize Quill with clean toolbar
